# Data Processing Addendum (DPA) de PymesHub

## 1. Proposito

Este Data Processing Addendum regula el tratamiento de datos personales que `Otnel S.A` realiza por cuenta del cliente al prestar PymesHub. Su finalidad es fijar con claridad roles, instrucciones, medidas de seguridad, uso de subprocesadores, asistencia al cliente y reglas de finalizacion del tratamiento.

## 2. Alcance

Este DPA aplica cuando el cliente actua como responsable del tratamiento respecto de datos personales que decide cargar o gestionar en PymesHub y `Otnel S.A` actua como encargado o subencargado para prestar el servicio.

## 3. Definiciones

- `Responsable`: cliente que determina fines y medios del tratamiento.
- `Encargado`: `Otnel S.A` cuando trata datos por cuenta del cliente.
- `Subencargado`: tercero contratado por `Otnel S.A` para apoyar el servicio.
...
El cliente actua como responsable respecto de los datos que decide incorporar a PymesHub. `Otnel S.A` actua como encargado al tratar esos datos para prestar el servicio. No obstante, `Otnel S.A` podra actuar como responsable independiente en actividades propias y necesarias como seguridad de la plataforma, facturacion, cumplimiento legal o defensa legal.

## 5. Objeto y duracion del tratamiento

`Otnel S.A` tratara datos personales durante la vigencia de la relacion contractual y por el tiempo adicional estrictamente necesario para:
...
La finalidad del tratamiento es exclusivamente prestar, soportar, asegurar y mantener PymesHub conforme al contrato.
...
`Otnel S.A` tratara los datos personales solo conforme a:
...
Si `Otnel S.A` considera que una instruccion infringe ley aplicable o compromete gravemente la seguridad o integridad del servicio, podra rechazarla o solicitar aclaracion.
...
`Otnel S.A` se asegurara de que su personal, contratistas y personas autorizadas para acceder a datos personales:
...
`Otnel S.A` aplicara medidas razonables y proporcionadas, incluyendo:
...
El cliente autoriza el uso de subencargados razonablemente necesarios para operar PymesHub. `Otnel S.A` impondrá a dichos terceros obligaciones apropiadas de confidencialidad y seguridad acordes al riesgo. La lista viva se mantiene en [`../security/subprocessors-list.md`](../security/subprocessors-list.md).

Cuando el cliente requiera un control contractual adicional sobre subencargados, ello debera pactarse expresamente.

## 13. Transferencias internacionales

Si un subencargado o sistema auxiliar trata datos fuera de `Costa Rica`, `Otnel S.A` procurara implementar salvaguardas contractuales u organizativas razonables segun la normativa y el riesgo aplicables.

## 14. Asistencia al cliente

En la medida razonable y tecnicamente viable, `Otnel S.A` asistira al cliente para:
...
Ante un incidente confirmado que afecte datos personales tratados por cuenta del cliente, `Otnel S.A` notificara sin demora indebida y compartira, en la medida razonablemente disponible:
...
Si `Otnel S.A` recibe directamente una solicitud de titular relativa a datos tratados por cuenta del cliente, podra:
...
Al finalizar el servicio, `Otnel S.A` permitira exportar, devolver o suprimir los datos del cliente conforme al contrato, la politica de retencion y las limitaciones tecnicas razonables. Los datos residuales en backups o registros de seguridad podran conservarse hasta su expiracion normal o mientras exista obligacion legal legitima.

## 18. Auditoria y evidencia

Sujeto a seguridad, confidencialidad y no interferencia operativa, `Otnel S.A` podra proporcionar evidencia razonable del cumplimiento de este DPA, priorizando:
