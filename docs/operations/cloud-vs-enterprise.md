...
- El despliegue es `single-tenant` y `Windows native` en un servidor local del cliente.
- La entrega principal es un servidor Windows con `PostgreSQL`, `API`, `Web` y storage local en disco.
...
- Los documentos viven en disco local administrado por la app, con metadata en PostgreSQL.
