 ...
 * the cart — "Change" — and `cart.promotion.apply` ("Apply") would be describing a
 ...
 * sentences and reached an English reader verbatim, because a Worker has no dictionary and no
 * translator. The API now answers with the **key** — `PROMOTION_ERROR_KEYS` in
 * `@pymeshub/shared` is the closed set, and neither locale may drift from it — and each client
 * renders it through its own translator. These six are the English for the Spanish originals
 * in `es/basket.ts`, which is the source of truth for the set.
 *
...
export const basket = {
	...
	"cart.promotion.change": "Change",
	/** The promo sheet's button that takes the code off the cart. */
	"cart.promotion.remove": "Remove code",
	/**
	 ...
	 */
	"cart.promotion.error.notFound": "That code doesn't exist",
	"cart.promotion.error.inactive": "That code is no longer active",
	"cart.promotion.error.notYetValid": "That code isn't valid yet",
	"cart.promotion.error.expired": "That code has expired",
	"cart.promotion.error.exhausted": "That code has been fully redeemed",
	"cart.promotion.error.belowMinimum":
		"Your order doesn't reach that code's minimum",
} as const;
