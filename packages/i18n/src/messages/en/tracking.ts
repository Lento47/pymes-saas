 ...
 *
 * `order.track.eta` already exists ("Arrives around {time}") and is deliberately **not**
 * reused here. It is built on the only estimate the API actually stores —
 * `estimatedReadyAt`, computed in `apps/api/src/services/mappers.ts` from the business's own
 * `prepTimeMinutes` and anchored at the moment the order was accepted — and "arrives" is a
 ...
 * "You have no orders yet" under a segment called *In progress* is false for somebody with
...
export const tracking = {
	 ...
	 */
	"tracking.estimate.ready.sameDay": "Ready around {time}",
	/**
	 * The same estimate when it falls on another day, which for a real prep time means the
	 * order crossed midnight. The day is in the sentence because "around 12:30" alone would
	 * not say which one.
	 */
	"tracking.estimate.ready.otherDay": "Ready on {date} at {time}",
	/**
	 ...
	 */
	"tracking.estimate.help": "That is an estimate, not an exact time.",

	/** The third payment method. See the note above — this key closes a silent omission. */
	"checkout.payment.card": "Card",

	/** The filter's "in progress" tab, empty. True whether or not there is a history. */
	"tracking.empty.active.title": "No orders in progress",
	"tracking.empty.active.body":
		"When you place an order, you follow it from here.",
	/** The filter's "past" tab, empty. Says where the finished ones will appear. */
	"tracking.empty.past.title": "No past orders yet",
	"tracking.empty.past.body": "Finished orders stay here, with their receipt.",
} as const;
