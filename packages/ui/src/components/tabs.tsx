...
const tabsListVariants = cva(
	// The list is the segmented control's well (`md`), each trigger sits in it
	// (`sm`). The `line` variant resets to square because it is bare labels with a
	// rule under the active one — there is no surface for a corner to belong to.
...
function TabsContent({
	...
	return (
		<TabsPrimitive.Content
			...
			// No `outline-none`: Radix gives the panel `tabIndex: 0`, so Tab reaches it and it
			// is a real focus stop. Suppressing the ring without a replacement is the one thing
			// `docs/design.md` forbids, and unlike `TabsTrigger` this element draws no ring of
			// its own — the global `:focus-visible` in `globals.css` is the fallback it takes.
