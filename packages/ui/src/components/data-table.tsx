...
import {
	...
	DropdownMenuTrigger,
} from "@pymeshub/ui/components/dropdown-menu";
import {
	InputGroup,
	InputGroupAddon,
	InputGroupInput,
} from "@pymeshub/ui/components/input-group";
import { Spinner } from "@pymeshub/ui/components/spinner";
import { TablePagination } from "@pymeshub/ui/components/table-pagination";
import {
	Table,
	TableBody,
	TableCell,
...
import { ROW_ACCENT, ROW_ACCENT_EXPANDABLE } from "@pymeshub/ui/lib/row-accent";
import type { TableQueryState } from "@pymeshub/ui/lib/table-query";
...
/**
 * Every word the table paints, supplied by the caller.
 *
 * `packages/ui` is the translation-free primitive layer — it has no `@pymeshub/i18n`
 * dependency and never will — so a word this file owns is a word in the wrong language
 * for somebody. It owned ten of them: `"Search…"`, `"Filters"`, `"Sort"`, `"Sort by"`,
 * `"Ascending"`, `"Descending"`, `"Columns"`, `"Toggle columns"`, `"No results found."`
 * and an `"All"` tab, every one English, in a Spanish-first product, on the screen an
 * operator reads all day.
 *
 * Required and total rather than optional with defaults, for the reason
 * `business-card`'s `badgeLabels` gives: a partial map hides the omission until a reader
 * hears the fallback, while a total one makes the next word added fail the build at
 * every call site instead of rendering as nothing.
 */
export type DataTableLabels = {
	/** The search box's placeholder. */
	searchPlaceholder: string;
	/** The phone-width button that opens the filter cluster. */
	filters: string;
	sort: string;
	sortBy: string;
	ascending: string;
	...
	/** Drawn in place of the rows when there are none; `empty` overrides it. */
	noResults: string;
	/**
	 * `TablePagination`'s two arrows, passed straight through — this file never defaults
	 * them, because its own default used to be English and the pagination's is Spanish.
	 *
	 * That component's third sentence, the `Mostrando 1–25 de 90` summary, is its own and
	 * stays its own: a caller with a dictionary hands it over as `meta`, the way
	 * `apps/web/components/admin/list-footer.tsx` does.
	 */
	previous: string;
	next: string;
};
...
export type DataTableTabs = {
	id: string;
	/**
	 * The word for the tab that filters nothing — "Todos", "All".
	 *
	 * A required field rather than the `?? "All"` that was here, because a tab group only
	 * exists when a caller passed one: the caller is already naming every option, so the
	 * label travels with the options it labels and a default could only ever be a second
	 * language in the same menu.
	 */
	allLabel: string;
	options: { value: string; label: string }[];
};
...
export type DataTableExpandable<TRow, TSub> = {
	...
	/**
	 * What the expand column is called: its `sr-only` header and its entry in the Sort
	 * menu.
	 *
	 * One word for both, required alongside `expandable` rather than living in `labels`,
	 * because only a caller that passes `expandable` has an expand column to name. It
	 * replaces two that disagreed in the same menu about the same column: the header said
	 * `"Detalle"` and the sort option said `"Detail"`.
	 */
	columnLabel: string;
};
...
export type DataTableProps<TRow, TSub> = {
	...
	loading?: boolean;
	/** The table's chrome, in the reader's language. See `DataTableLabels`. */
 ...
 * `expand` (which rows are open) and `hide` (which columns are off). Every word it
 * paints, it does not own: they arrive in `labels`, because this package has no
 * dictionary to answer with.
 */
							...
							<DropdownMenuContent align="start" className="min-w-52">
								...
								>
									<DropdownMenuRadioItem value="all">
										<span className="flex-1">{tabs.allLabel}</span>
									</DropdownMenuRadioItem>
					...
					<TableRow>
						{anyExpandable && (
							<TableHead className="h-11 w-10 px-3">
								<span className="sr-only">{expandable?.columnLabel}</span>
							</TableHead>
						)}
				...
				<TableBody>
					{deferredRows.length === 0 ? (
						<TableRow className="hover:bg-transparent">
							...
							>
								{loading ? <Spinner /> : (empty ?? labels.noResults)}
							</TableCell>
						</TableRow>
					) : (
