...
export class CsvImportController {
  ...
  async parseContactsCsv(@UploadedFile() file: Express.Multer.File) {
    ...
    for (const h of headers) {
      ...
      if (lower.includes("nombre") || lower.includes("name") || lower.includes("full"))
        suggestedMapping.full_name = h;
      else if (lower.includes("email") || lower.includes("correo")) suggestedMapping.email = h;
      else if (lower.includes("telefono") || lower.includes("phone") || lower.includes("tel"))
        suggestedMapping.phone = h;
      else if (lower.includes("empresa") || lower.includes("company") || lower.includes("razon"))
        suggestedMapping.company_name = h;
  ...
  async confirmContactsImport(
    ...
    if (!data.mapping || !data.rows?.length)
      throw new BadRequestException("Mapping y rows requeridos");
  ...
  async parseInvoicesCsv(@UploadedFile() file: Express.Multer.File) {
    ...
    for (const h of headers) {
      ...
      else if (lower.includes("monto") || lower.includes("amount") || lower.includes("total"))
        suggestedMapping.amount = h;
      else if (lower.includes("cliente") || lower.includes("contact") || lower.includes("nombre"))
        suggestedMapping.contact_name = h;
      else if (lower.includes("fecha") || lower.includes("venc")) suggestedMapping.due_date = h;
      else if (lower.includes("moneda") || lower.includes("currency"))
        suggestedMapping.currency = h;
  ...
  async parseProductsCsv(@UploadedFile() file: Express.Multer.File) {
    ...
    for (const h of headers) {
      ...
      if (lower.includes("nombre") || lower.includes("name") || lower.includes("producto"))
        suggestedMapping.name = h;
      else if (lower.includes("sku") || lower.includes("codigo") || lower.includes("código"))
        suggestedMapping.sku = h;
      else if (lower.includes("precio") || lower.includes("price") || lower.includes("unitario"))
        suggestedMapping.unit_price = h;
        ...
        suggestedMapping.current_stock = h;
      else if (lower.includes("minimo") || lower.includes("mínimo") || lower.includes("min"))
        suggestedMapping.min_stock = h;
      else if (
      ...
      else if (lower.includes("unidad") || lower.includes("medida") || lower.includes("unit"))
        suggestedMapping.unit_of_measure = h;
      else if (lower.includes("descrip") || lower.includes("detalle"))
        suggestedMapping.description = h;
    }
