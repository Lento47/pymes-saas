...
export class ContactSalesController {
  ...
  @Roles('OWNER', 'ADMIN')
  async getInquiries(@CurrentUser('workspace_id') workspaceId: string, @Query('status') status?: string) {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('OWNER', 'ADMIN')
  async updateStatus(@CurrentUser('workspace_id') workspaceId: string, @Param('id') id: string, @Body('status') status: string) {
