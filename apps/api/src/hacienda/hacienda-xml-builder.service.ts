...
<${rootElement}>
  ...
  private computeResumen(lines: Record<string, any>[], invoice: Record<string, any>) {
    ...
    for (const line of lines) {
      ...
      if (taxRate > 0) {
        isService ? (totalServGravados += subtotal) : (totalMercGravados += subtotal);
      } else {
        isService ? (totalServExentos += subtotal) : (totalMercExentos += subtotal);
      }
    }
  ...
  private buildReferenciaXml(
    refInvoice: Record<string, any>,
    currentInvoice: Record<string, any>,
  ): string {
    const razon =
