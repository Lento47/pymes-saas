...
export class AutomationsController {
  ...
  async create(
    ...
    if (dto.condition_config_json && Object.keys(dto.condition_config_json).length > 0) {
      await this.planLimits.enforcePlanTier(
        workspaceId,
        "GROWTH",
        "Constructor avanzado de condiciones",
      );
    }
  ...
  findOne(@CurrentUser() user: Record<string, any>, @Param("id", ValidateUUIDPipe) id: string) {
  ...
  @Roles(WorkspaceUserRole.ADMIN, WorkspaceUserRole.OWNER)
  @RequireFeature("automations")
  toggle(@CurrentUser() user: Record<string, any>, @Param("id", ValidateUUIDPipe) id: string) {
  ...
  @Roles(WorkspaceUserRole.ADMIN, WorkspaceUserRole.OWNER)
  @RequireFeature("automations")
  remove(@CurrentUser() user: Record<string, any>, @Param("id", ValidateUUIDPipe) id: string) {
