...
export async function generateBillingInvoicePdf(invoice: BillingInvoicePdfData): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    let ry = 62;
    doc.text(`N° ${invoice.number}`, M, ry, { width: W, align: "right", lineBreak: false });
    ry += 11;
    doc.text(
      `Emitida: ${invoice.issuedAt.toLocaleDateString("es-CR", { year: "numeric", month: "long", day: "numeric" })}`,
      M,
      ry,
    ...
    if (invoice.dueDate) {
      ry += 11;
      doc.text(
        `Vence: ${invoice.dueDate.toLocaleDateString("es-CR", { year: "numeric", month: "long", day: "numeric" })}`,
        M,
        ry,
        { width: W, align: "right", lineBreak: false },
      );
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(6.5)
      .text("FACTURAR A", M + 12, y + 9);
    let cy = y + 22;
    const fields = [invoice.clientName, invoice.clientEmail, invoice.clientCompany].filter(
      Boolean,
    ) as string[];
    fields.forEach((f) => {
      const isEmail = f.includes("@");
      doc
        .font(isEmail ? "Helvetica" : "Helvetica-Bold")
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(6.5)
      .text("PLAN", planX + 12, y + 9);
    doc
      .fillColor(BRAND.dark)
      .font("Helvetica-Bold")
      .fontSize(10)
      .text(invoice.planName, planX + 12, y + 22);
    doc
      .fillColor(BRAND.gray)
      .font("Helvetica")
      .fontSize(7)
      .text(
        `${invoice.planInterval === "MONTHLY" ? "Mensual" : "Anual"} · ${invoice.seats} usuario(s)`,
        planX + 12,
        y + 36,
      );
    ...
    invoice.lineItems.forEach((item, idx) => {
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .text(fmt(item.total), M, y + 4, { width: W, align: "right" });
      y += rowH + 2;
    });
    ...
    if (invoice.taxRate > 0) {
      y += 20;
      doc
        .text(`IVA (${invoice.taxRate}%)`, M, y, { width: totLabelW })
    ...
    if (invoice.notes) {
      ...
      doc
        ...
        .font("Helvetica")
        .fontSize(8)
        .text(invoice.notes, M, y + 12, { width: W });
    }
    ...
    doc
      ...
      .text("support@pymeshub.lat  ·  PymesHub.lat  ·  Factura electrónica", M + 26, footerY + 18, {
        width: W - 26,
        align: "center",
      });
