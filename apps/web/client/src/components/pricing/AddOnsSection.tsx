...
export function AddOnsSection({ addOns, billingPeriod }: AddOnsSectionProps) {
    ...
    onSuccess: (data) => {
      if (data?.updated) {
        toast({
          ...
          description:
            'Tu suscripción se actualizó con los usuarios extra. El cargo prorrateado se aplica de inmediato.',
        });
  ...
  const handleAdd = (addOn: AddOn) => {
    const isPurchasable = !!addOn.priceKeyMonthly;
    if (!isPurchasable) {
      ...
      {addOns.map((addOn) => {
        ...
        return (
          <div key={addOn.key} className="rounded-lg border border-gray-200 bg-white p-6">
            ...
            >
              {loading ? 'Procesando…' : isPurchasable ? 'Agregar al plan' : 'Próximamente'}
            </Button>
          </div>
        );
      })}
