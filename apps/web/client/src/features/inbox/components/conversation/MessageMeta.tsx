...
export function MessageMeta({ message, isOutbound, showSenderName, compact, overlay, isLargeEmoji }: MessageMetaProps) {
  if (overlay) {
    return (
  ...
  // ── Outbound: white text on indigo bg (muted when large-emoji has no bg) ──
  if (isOutbound) {
    const timeColor = isLargeEmoji ? "text-muted-foreground/60" : "text-white/50";
    const cls = compact
      ? `text-[9px] ${timeColor} mt-0.5 text-right justify-end flex items-center gap-1`
      : `text-[10px] ${timeColor} mt-1.5 text-right justify-end flex items-center gap-1`;
    return (
      <div className={cls}>
        <span>{formatMessageTime(message.sentAt)}</span>
        <MessageStatus
